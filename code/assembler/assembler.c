	...
	while(file < (numLib+1))
	{
		...
		while(pc < 256)
		{
			...
			if(flag == 1)
			{
				...
				{
					//Salva o .data ou .extern em uma lista data
					token = strtok(NULL, " \t");
					line[strlen(line)-1] = '\0'; // Retirando o : do line
					adiciona_elemento(&data, data.ultima);
					grava_elemento(data.ultima, line, pc, file);
				}

				else if(strcmp(token, ".extern") == 0) // Identifica uma pseudoinstrução .extern
				{
					//Salva o .extern em uma lista ext
					token = strtok(NULL, " \t");
					adiciona_elemento(&ext, ext.ultima);
					grava_elemento(ext.ultima, token, pc, file);
				}
			}
